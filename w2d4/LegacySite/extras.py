...
def parse_card_data(card_file_data, card_path_name):
    ...
    try:
        ...
        pass
    with open("tmp_file", "wb") as card_file:
        card_file.write(card_file_data)

    print(f"running: {CARD_PARSER} 2 tmp_file > tmp_file2")
    ret_val = system(f"{CARD_PARSER} 2 tmp_file > tmp_file2")
    if ret_val != 0:
        return card_file_data
    with open("tmp_file2", "rb") as tmp_file:
