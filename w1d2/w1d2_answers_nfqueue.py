...
def process_packet(packet):
    ...
    try:
        parsed_packet_payload = IP(packet.get_payload())
        if parsed_packet_payload.dst in BLOCKED_IPS:
            print(parsed_packet_payload)
            packet.drop()
        else:
            packet.accept()
    except Exception:
...
print("Starting packet filter in Python...")
nfqueue = NetfilterQueue()
nfqueue.bind(0, process_packet)
try:
    nfqueue.run()
except KeyboardInterrupt:
    print("Stopping packet filter in Python...")
finally:
    nfqueue.unbind()
