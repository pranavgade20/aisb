...
class DNSInterceptor:
    ...
    def dns_request(self, flow: mitmproxydns.DNSFlow) -> None:
        ...
        print(f"flow req: {flow.request}")
        if flow.request != None: 
            print(f"got a query")
            for q in flow.request.questions:
                print(f"iterating over q{q}")
                if q.type == 16:
                    print(f"q.name is{q.name}")
                    if "evil.aisb.dev" in q.name:
                        print("caught sus TXT query")
