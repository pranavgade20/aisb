...
class RequestBlocker:
    def request(self, flow: http.HTTPFlow) -> None:
        ...
        if "evil.aisb.dev" in flow.request.pretty_host:
            flow.response = http.Response.make(200, b"intercepted and blocked request :)")
...
class DNSInterceptor:
    ...
    def dns_request(self, flow: mitmproxydns.DNSFlow) -> None:
        ...
        TXT_REC_VAL = 16
        blocked_domains = ["evil.aisb.dev"]
        if flow.request:
            for q in flow.request.questions:
                if q.type == TXT_REC_VAL:
                    if q.name in blocked_domains:
                        flow.response = get_packet(q, flow.request)
                        print("DNS request blocked")
