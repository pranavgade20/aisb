...
def md5_padding(message: bytes) -> bytes:
    ...
    mod_len = msg_len % 64
    app_len = (56 - mod_len - 1) % 64
    print(f'app_len {app_len}')
    for _ in range(app_len):
        message += b"\x00"
    ...
    message += int64_to_bytes_le(msg_len_bits)
...
def md5_process_block(block: bytes, state: List[int]) -> List[int]:
    ...
    for i in range(16):
        offset = 4 * i
        words_32bit.append(bytes_to_int32_le(block, offset=offset))
    ...
    for i in range(64):
        ...
        temp = A + fn(B, C, D) + words_32bit[k] + MD5_T[i]
        temp = mask_lower_32(temp)
