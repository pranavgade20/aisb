...
def md5_padding(message: bytes) -> bytes:
    ...
    message = bytearray(list(message) + [0x80])

    remainder = len(message) % 64
    if remainder <= 56:
        padding_length = 56 - remainder
    else:
        padding_length = (64 - remainder) + 56

    padded_message = message + bytearray([0x0] * padding_length)
    padded = padded_message + (original_message_length * 8).to_bytes(8, "little")
    return padded
...
def md5_process_block(block: bytes, state: List[int]) -> List[int]:
    ...
    X = []
    for i in range(16):
        X.append(bytes_to_int32_le(block, i*4))
    ...
    for i in range(64):
        ...
        A, B, C, D = D, (B + left_rotate((A + F(B,C,D) + X[k] + MD5_T[i]), MD5_S[i])) & 0xFFFFFFFF, B, C
...
def md5_hash(message: bytes) -> bytes:
    ...
    for i in range(len(padded_message) // 64):
        state = md5_process_block(padded_message[i*64:i*64+64], state)

    all_bytes = [i.to_bytes(4, "little") for i in state]
    return all_bytes[0] + all_bytes[1] + all_bytes[2] + all_bytes[3]
...
def length_extension_attack(
    ...
    padded_message = md5_padding(b'\1' * (len(original_message) + secret_length))
    glue_padding = padded_message[len(original_message)+secret_length:]

    forged_message = original_message + glue_padding + additional_data

    internal_state = []
    for i in range(4):
        internal_state.append(bytes_to_int32_le(original_tag, i*4))

    final_padded_message = md5_padding(b'\1' * (secret_length + len(original_message) + len(glue_padding) + len(additional_data)))
    final_padding = final_padded_message[len(original_message)+secret_length+len(glue_padding)+len(additional_data):]

    data = additional_data + final_padding
    for i in range(len(data) // 64):
        internal_state = md5_process_block(data[i*64:(i+1)*64], internal_state)

    all_bytes = [i.to_bytes(4, "little") for i in internal_state]
    return (forged_message, all_bytes[0] + all_bytes[1] + all_bytes[2] + all_bytes[3])
