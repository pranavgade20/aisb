...
def md5_process_block(block: bytes, state: List[int]) -> List[int]:
    ...
    for i in range(len(block)):
        if i % 4 == 0:
            word = bytes_to_int32_le(block, i)
    ...
    #     print(f"type of state[i] after  {type(state[i])}")
    #     state[i] = mask_32(state[i])
    #     print(f"type of state[i] {type(state[i])}")
...
def naive_verify(message: bytes, secret: bytes, tag: bytes) -> bool:
    ...
    check_tag = naive_mac(message, secret)
    if check_tag == tag:
        return True
    return False
...
def length_extension_attack(
    ...
    evil_message = original_message + glue_padding + additional_data

    og_state = [0] * 4
    og_state[0] = bytes_to_int32_le(original_tag, 0)
    og_state[1] = bytes_to_int32_le(original_tag, 32)
    og_state[2] = bytes_to_int32_le(original_tag, 64)
