...
def recover_seed(ciphertext: bytes, known_plaintext: bytes, position: int) -> int:
    ...
    if "SOLUTION":
        ...
        keystream_at_position = [ciphertext[position + i] ^ known_plaintext[i] for i in range(len(known_plaintext))]
...
def meet_in_the_middle_attack(plaintext: bytes, ciphertext: bytes) -> List[Tuple[int, int]]:
    ...
    for k1 in range(0, 1024):
        encrypted = des_encrypt(k1, plaintext)
        key = int.from_bytes(encrypted, byteorder="big")
        if key not in forwardtbl:
            forwardtbl[key] = []
        forwardtbl[key].append(k1)

    for k2 in range(0, 1024):
        decrypted = des_decrypt(k2, ciphertext)
        key = int.from_bytes(decrypted, byteorder="big")
        if key in forwardtbl:
            for k1 in forwardtbl[key]:
                res.append((k1, k2))
    print(res)
    return res
